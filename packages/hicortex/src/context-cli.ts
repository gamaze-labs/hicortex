...
import { isValidSectionName, isValidAgentId, SECTION_NAME_MAX } from "./context-store.js";
...
export class ContextCliError extends Error {}
 ...
 */
export function resolveContextTarget(config: Record<string, unknown>): ContextServerTarget {
  const baseUrl =
...
function contextUrl(baseUrl: string, agent?: string): string {
...
function describeFailure(status: number | null, bodyError?: string): string {
  ...
  if (status === 404) return `HTTP 404 — /context not found; the server is likely too old (needs 0.12+)${suffix}`;
...
export async function getContext(target: ContextServerTarget, agent?: string): Promise<ContextGetResponse> {
  let resp: Response;
  try {
  ...
  const data = (await resp.json()) as ContextGetResponse;
  ...
  if (agent && data.agent !== agent) {
    throw new ContextCliError(
      `server did not echo agent '${agent}' — it likely predates per-agent context (needs 0.13+)`,
    );
  }
  return data;
}
...
export async function putSection(target: ContextServerTarget, name: string, content: string, agent?: string): Promise<void> {
  let resp: Response;
  try {
    resp = await fetch(contextUrl(target.baseUrl, agent), {
      method: "PUT",
  ...
  if (agent) {
    ...
    if (body.agent !== agent) {
      throw new ContextCliError(
        `PUT /context?agent=${agent} — server did not echo the agent; it likely predates per-agent ` +
...
export function formatAllSections(data: ContextGetResponse): string {
...
export function formatOneSection(data: ContextGetResponse, name: string): string | null {
...
export async function runEdit(name: string, spawn: EditorSpawn = defaultSpawn, agent?: string): Promise<void> {
  ...
  const dir = mkdtempSync(join(tmpdir(), "hicortex-ctx-"));
...
export async function runContextCommand(args: string[]): Promise<void> {
  ...
  switch (sub) {
    case "show":
      ...
      return;
    case "edit":
      if (!rest[1]) throw new ContextCliError("Usage: hicortex context edit <name> [--agent <id>]");
